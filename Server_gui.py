...
class Ui_MainWindow(object):
    ...
    def server_thread(self):
        ...
        print("sending private, public keys and modulus: ")
        print(public_key)
        print(private_key)
        print(modulus)
    ...
    def send_key_data(self):
        global unlockCar
        encrypted = rsa_library.encrypt(self.pub_key, unlockCar)
        print("sending encrypted key: ", encrypted)
        self.conex.send(str(encrypted).encode())
    ...
    def recv_messages_handler(self, stop_event):
        while True:
            message = self.conex.recv(1024).decode()
            print("message: ", message)
            # print("type message: ", type(message))
            decrypted_message = rsa_library.decrypt(self.pri_key, message)
            print("NEW MESSAGE:", decrypted_message)
            if rsa_library.low_check(decrypted_message) and rsa_library.number_check(decrypted_message):
                self.conex.send(str(rsa_library.encrypt(self.pub_key, "0x69")).encode())  # 0x69 = all g YES
                self.ecu_defect_label.setVisible(False)
                self.airbag_label.setVisible(True)
            elif not rsa_library.low_check(decrypted_message):
                self.airbag_label.setVisible(False)
                self.ecu_defect_label.setVisible(True)
                self.ecu_defect_label.setText('  ECU\nDefect')
                self.conex.send(str(rsa_library.encrypt(self.pub_key, "0x00")).encode())  # 0x00 = err
            else:
                self.airbag_label.setVisible(False)
                self.ecu_defect_label.setVisible(True)
                self.ecu_defect_label.setText('  ECU\nDefect')
                self.conex.send(str(rsa_library.encrypt(self.pub_key, "0x1")).encode())  # 0x00 = err
